...
class ClassificationService:
    ...
    def _is_lora_model(self, model_path: str) -> bool:
        """Provjeri je li model LoRA adapter (ima adapter_config.json)."""
        return (Path(model_path) / "adapter_config.json").exists()
 
    def _load_model(self, model_path: str):
        """Učitaj model – detektira automatski full FT vs LoRA."""
        if self._is_lora_model(model_path):
            print(f"Detected LoRA adapter, loading with PeftModel...")
